...
export default function MisInformes() {
  ...
  useEffect(() => {
    ...
    // "activos" no es un valor válido del backend, lo gestionamos client-side
    const url =
      filtro === "activos" || filtro === "todos"
        ? "/informes"
        : `/informes?estado=${filtro}`;

    apiFetch<Informe[]>(url)
      .then((data) => {
        if (filtro === "activos") {
          setInformes(
            data.filter(
              (i) => i.estado === "PENDIENTE" || i.estado === "DEVUELTO",
            ),
          );
        } else {
          setInformes(data);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Select value={filtro} onValueChange={setFiltro}>
          ...
          <SelectContent>
            <SelectItem value="activos">Activos (en curso)</SelectItem>
        ...
        <CardContent>
          {loading ? (
            ...
            <p className="text-muted-foreground">
              {filtro === "activos"
                ? "No tienes informes en curso. ¡Bien hecho!"
