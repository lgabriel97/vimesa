...
<body>
  ...
  <table>
    ...
    <tr>
      ...
      <td>${v(datos.noOrden)}</td>
      <td class="label">N/S:</td>
      <td>${v(datos.nSerie)}</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>${v(datos.cliente)}</td>
      <td class="label">Sitio:</td>
      <td>${v(datos.sitio)}</td>
    </tr>
    <tr>
      <td colspan="2">
        <div>${check(datos.tipoEquipo === "NUEVO")} Equipo nuevo</div>
        <div>${check(datos.tipoEquipo === "REPARADO_FABRICA")} Equipo reparado en fábrica &nbsp;&nbsp; ${check(datos.tipoEquipo === "REPARADO_VIMESA")} Equipo reparado en Vimesa</div>
      </td>
      <td class="label">Observaciones:</td>
      <td>${v(datos.observaciones)}</td>
    </tr>
    <tr>
      <td colspan="4" style="text-align: center; font-weight: 700;">
        Lectura de parámetros a temperatura ambiente de: ${v(datos.tempAmbiente)} ºC
      </td>
    </tr>
    <tr>
  ...
  <table>
    ...
    <tr>
      <td style="width: 50%;">
        ...
        <div style="margin-top: 4px;"><b>Versión Firmware:</b> ${v(datos.versionFirmware)}</div>
      </td>
      <td style="width: 50%; vertical-align: top;">
        <div><b>IP SNMP V1:</b> ${v(datos.snmpV1?.ip)}</div>
        <div><b>Password:</b> ${v(datos.snmpV1?.password)}</div>
        <div><b>Mask:</b> ${v(datos.snmpV1?.mask)}</div>
        <div><b>Gateway:</b> ${v(datos.snmpV1?.gateway)}</div>
        <div style="margin-top: 4px;"><b>IP SNMP V2:</b> ${v(datos.snmpV2?.ip)}</div>
        <div><b>Login:</b> ${v(datos.snmpV2?.login)} &nbsp;&nbsp; <b>Password:</b> ${v(datos.snmpV2?.password)}</div>
        <div><b>Mask:</b> ${v(datos.snmpV2?.mask)}</div>
        <div><b>Gateway:</b> ${v(datos.snmpV2?.gateway)}</div>
        <div style="margin-top: 4px;"><b>Versión Web Server:</b> ${v(datos.versionWebServer)}</div>
      </td>
    </tr>
  </table>
